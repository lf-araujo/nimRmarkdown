spinnim <- function(nimfile, text=NULL, keep=FALSE, ...) {
    # stopifnot((length(nimfile)==1 && file.exists(nimfile))||length(text)==1)
    if (is.null(text)) {
        vtext <- readLines(nimfile, warn=FALSE)
    } else {
    ...
    nim_code <- (md_block + chunk_head + R_code) == 0              # nim code lines
...
#    vtext[chunkl] <- paste0(vtext[chunkl], "\n")        # ensure trailing \n
    ...
    # return(data.frame(vtext, md_block, chunk_head, R_code, nim_code))

    
    if (is.null(text)) {
        rfile <- sub("[.]do$", ".r", nimfile)
        
        writeLines(vtext, rfile)
        if (!keep)
            on.exit(unlink(rfile), add=TRUE)
        # knitr::spin(rfile, precious=keep, comment=c("^/[*][*]", "^.*[*]/[*] *$"), ...)
        spin_lang(rfile, precious=keep, comment=c("^/[*][*]", "^.*[*]/[*] *$"), language="nim", ...)
    } else {
        # return(knitr::spin(text=vtext, precious=keep, comment=c("^/[*][*]", "^.*[*]/[*] *$"), ...))
        return(spin_lang(text=vtext, precious=keep, comment=c("^/[*][*]", "^.*[*]/[*] *$"), language="nim", ...))
    }
